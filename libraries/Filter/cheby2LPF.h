...
public:
    cheby2LPF();
    // constructor
    cheby2LPF(float sample_freq, float cutoff_freq);
    // change parameters
...
protected:
    struct user_DigitalLPF<T>::biquad_params _params;
    
private:
    user_DigitalLPF<T> _filter;
};
...
typedef cheby2LPF<int>      cheby2LPFInt;
typedef cheby2LPF<long>     cheby2LPFLong;
typedef cheby2LPF<float>    cheby2LPFFloat;
typedef cheby2LPF<Vector2f> cheby2LPFVector2f;
typedef cheby2LPF<Vector3f> cheby2LPFVector3f;
