...
public:
    LPFrd();
    // constructor
    LPFrd(float sample_freq, float cutoff_freq);
    // change parameters
...
protected:
    struct DigitalBiquadFilterRD<T>::biquad_params _params;
    
private:
    DigitalBiquadFilterRD<T> _filter;
};
...
typedef LPFrd<int>      LPFrdInt;
typedef LPFrd<long>     LPFrdLong;
typedef LPFrd<float>    LPFrdFloat;
typedef LPFrd<Vector2f> LPFrdVector2f;
typedef LPFrd<Vector3f> LPFrdVector3f;