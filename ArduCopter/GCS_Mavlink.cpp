...
void Copter::send_cass_hyt271(mavlink_channel_t chan) {
    ...
    // Variables simulation for HYT271 humidity sensors
        ...
        uint32_t m = AP_HAL::millis();
        raw_sensor[0] = simH + sinf(0.1f*float(m)) * 0.02;
        raw_sensor[1] = simH + sinf(0.1f*float(m)) * 0.03;
        raw_sensor[2] = simH + sinf(0.1f*float(m)) * 0.025;
        raw_sensor[3] = simH + sinf(0.1f*float(m)) * 0.028;
